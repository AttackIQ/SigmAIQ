    ...
    assert (
        SigmAIQPipeline(
            processing_pipeline=ProcessingPipeline(
                ...
                items=[
                    ProcessingItem(
                        identifier="test_id", transformation=FieldMappingTransformation({"FieldA": "FieldB"})
                    )
                ],
            )
        )
...
@pytest.mark.parametrize("available_pipeline", list(SigmAIQPipeline.display_available_pipelines().keys()))
...
def test_pipeline_factory_from_invalid_str():
    ...
    with pytest.raises(
        ...
        SigmAIQPipeline(processing_pipeline="invalid_pipeline_name").create_pipeline()
...
def test_pipeline_factory_no_pipeline():
    ...
    with pytest.raises(ValueError, match="Please provide a valid processing pipeline value to processing_pipeline"):
...
def test_pipeline_factory_from_callable():
    ...
    def pipeline_callable():
        return ProcessingPipeline(
            ...
            priority=10,
            items=[
                ProcessingItem(identifier="test_id", transformation=FieldMappingTransformation({"FieldA": "FieldB"}))
            ],
        )

    assert SigmAIQPipeline(processing_pipeline=pipeline_callable).create_pipeline().name == "test"
...
def test_pipeline_factory_from_invalid_callable():
    ...
    with pytest.raises(
        TypeError,
        match="Invalid processing_pipeline, " "or invalid object returned from callable passed as processing_pipeline",
    ):
...
def test_pipeline_factory_from_fieldmapping():
    ...
    pipeline = SigmAIQPipeline.from_fieldmap(fieldmapping={"FieldA": "FieldB"}, priority=10).create_pipeline()
...
def test_pipeline_factory_from_fieldmapping_empty():
    ...
    with pytest.raises(InvalidCustomFieldMapping, match="The provided field_mappings is empty or None."):
        SigmAIQPipeline.from_fieldmap(fieldmapping={}, priority=10).create_pipeline()


@pytest.mark.parametrize(
    "oob_priority", [-5, 105, None, "string"], ids=["less_than_0", "greater_than_100", "None", "a_string"]
)
def test_pipeline_factory_from_fieldmapping_priority_oob(oob_priority):
    ...
    if isinstance(oob_priority, str):
        with pytest.raises(TypeError, match="custom_field_mappings_priority must be of type `int`"):
            SigmAIQPipeline.from_fieldmap(fieldmapping={"FieldA": "FieldB"}, priority=oob_priority).create_pipeline()
    else:
        pipeline = SigmAIQPipeline.from_fieldmap(
            fieldmapping={"FieldA": "FieldB"}, priority=oob_priority
        ).create_pipeline()
...
def test_pipeline_factory_invalid_name():
    with pytest.raises(TypeError, match="'name' given for new fieldmapping pipeline must be of type str"):
        SigmAIQPipeline.from_fieldmap({"fieldA": "fieldB"}, name=0)
...
def test_pipeline_factory_from_invalid_fieldmapping():
    ...
    with pytest.raises(TypeError, match="fieldmappings must be of type 'dict' with str keys and values or None"):
        SigmAIQPipeline.from_fieldmap({"FieldA": 5})
        ...
        SigmAIQPipelineResolver(
            processing_pipelines=[
                ProcessingPipeline(
                    ...
                    items=[
                        ProcessingItem(
                            identifier="test_id1", transformation=FieldMappingTransformation({"FieldA": "FieldB"})
                        )
                    ],
                ),
                ProcessingPipeline(
                    ...
                    items=[
                        ProcessingItem(
                            identifier="test_id2", transformation=FieldMappingTransformation({"FieldA": "FieldB"})
                        )
                    ],
                ),
            ]
...
def test_pipeline_factory_resolver_invalid_pipeline():
    ...
    with pytest.raises(TypeError, match="processing_pipelines is not of type list or set"):
...
def test_pipeline_factory_resolver_no_pipelines():
    ...
    with pytest.raises(ValueError, match="processing_pipelines is empty or None"):
